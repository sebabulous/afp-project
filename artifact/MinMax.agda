...
open import artifact.DeletionUpdate using (balanceR; balanceL)

-- The minimal key of the map. Returns Nothing if the map is empty.
lookupMin : {K V : Set} → Map K V → Maybe (Pair K V)
...
lookupMax : {K V : Set} → Map K V → Maybe (Pair K V)
...
updateMaxWithKey : {K V : Set} → {{Comparable K}} → (K → V -> Maybe V) → Map K V → Map K V
...
-- Update the value at the minimal key.
updateMin : {K V : Set} → {{Comparable K}} → (V → Maybe V) → Map K V → Map K V
updateMin f m = updateMinWithKey (λ _ x → f x) m

-- Update the value at the maximal key.
updateMax : {K V : Set} → {{Comparable K}} → (V -> Maybe V) → Map K V → Map K V
...
minViewWithKey : {K V : Set} → {{Comparable K}} → Map K V → Maybe (Pair (Pair K V) (Map K V))
...
-- and the map stripped of that element, or Nothing if passed an empty map.
maxViewWithKey : {K V : Set} → {{Comparable K}} → Map K V → Maybe (Pair (Pair K V) (Map K V))
...
-- and the map stripped of that element, or Nothing if passed an empty map.
minView : {K V : Set} → {{Comparable K}} → Map K V → Maybe (Pair V (Map K V))
...
-- and the map stripped of that element, or Nothing if passed an empty map.
maxView : {K V : Set} → {{Comparable K}} → Map K V → Maybe (Pair V (Map K V))
maxView m with maxViewWithKey m 
... | nothing = nothing
... | just ((_ , x) , m₁) = just (x , m₁)

-- Delete and find the minimal element.
deleteFindMin : {K V : Set} → {{Comparable K}} → Map K V → Pair (Pair K V) (Map K V)
deleteFindMin m with minViewWithKey m
... | nothing = {!  error !}
... | just m₁ = m₁

-- Delete and find the maximal element.
deleteFindMax : {K V : Set} → {{Comparable K}} → Map K V → Pair (Pair K V) (Map K V)
