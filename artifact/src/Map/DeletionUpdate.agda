...
-- delete _ tip = nothing
delete k (node _ k' v tip tip) with compare k k'
...                            | eq = just tip
...                            | _ = nothing
delete k (node _ k' v l r) with (l , r , compare k k')
...                        | (tip , _ , lt) = nothing
...                        | (node _ _ _ _ _ , _ , lt) with delete k l
...                            | nothing = nothing
...                            | just l' = just (balanceR k' v l' r)
delete k (node _ k' v l r) | (_ , tip , gt) = nothing
delete k (node _ k' v l r) | (_ , node _ _ _ _ _ , gt) with delete k r
...                                                    | nothing = nothing
...                                                    | just r' = just (balanceL k' v l r')
delete k (node _ k' v l r) | (_ , _ , eq) = just (glue l r)
-- delete : {{Comparable K}} → K → Map K A (suc n) → Map K A n
-- delete _ tip = tip
...
updateWithKey f k (node s k' v l r) | gt with updateWithKey f k r
...                                      | modDelete r' = modDelete (balanceL k' v l r')
...                                      | modInsert r' = modInsert (balanceL k' v l r')
...                                      | modId r' = modId (balanceL k' v l r')
updateWithKey f k (node s k' v l r) | eq with f k' v
...                                      | just v' = modId (node s k' v' l r)
...                                      | nothing = modDelete (glue l r)
-- updateWithKey : {K V : Set} → {{Comparable K}} → (K → V → Maybe V) → K → Map K V n → Map K V n
...
...                                             | just v' = just v' :*: modId (node s k' v' l r)
...
alter f k (node s k' v l r) | gt with alter f k r
...                              | modId r' = modId (balance k' v l r')
...                              | modDelete r' = modDelete (balance k' v l r')
...                              | modInsert r' = modInsert (balance k' v l r')
alter f k (node s k' v l r) | eq with f (just v)
...                              | just v' = modId (node s k' v' l r)
...                              | nothing = modDelete (glue l r)
-- alter : {K V : Set} → {{Comparable K}} → (Maybe V → Maybe V) → K → Map K V n → Map K V n
-- alter f k tip with f nothing
-- ...           | nothing = tip
