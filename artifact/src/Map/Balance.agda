...
-- balance k v tip r@(node rk rv tip rr@(node _ _ _ _)) = node rk rv (node k v tip tip) rr
balance k v tip r@(node rk rv tip rr@(node _ _ _ _)) = balance rk rv (node k v tip tip) rr
-- balance k v tip (node rk rv (node rlk rlv rll rlr) tip) = node rlk rlv (node k v tip tip) (node rk rv tip tip)
...
-- balance k v (node lk lv tip (node lrk lrv _ _)) tip = node lrk lrv (node lk lv tip tip) (node k v tip tip)
...
-- balance k v l@(node lk lv ll lr) (node rk rv rl@(node rlk rlv rll rlr) rr@(node _ _ _ _)) | (gt,_) = if (size rl) < ratio * (size rr) then node rk rv (node k v l rl) rr else node rlk rlv (node k v l rll) (node rk rv rlr rr)
...
-- balance k v l@(node lk lv ll@(node _ _ _ _) lr@(node lrk lrv lrl lrr)) r@(node rk rv rl rr) | (_,gt) = if (size lr) < ratio * (size ll) then node lk lv ll (node k v lr r) else node lrk lrv (node lk lv ll lrl) (node k v lrr r)
